...
function hb_local_search($DB, $question, $words, $base_url, $docs_path) {
   $scores = [];
   $titles = [];
   $answers = [];
   foreach ($words as $w) {
      $weight = mb_strlen($w);
      $we = $DB->escape($w);
      $iter = $DB->doQuery("SELECT id, name, answer FROM glpi_knowbaseitems WHERE name LIKE '%$we%' OR answer LIKE '%$we%'");
      if (!$iter) continue;
...
if ($do_local) {
   ...
   $candidates = hb_local_search($DB, $question, $words, $base_url, $DOCS_PATH);
   if (empty($candidates)) {
